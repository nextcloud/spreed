    ...
    var ChatView = Marionette.View.extend({
        ...
        msgDraft: {
            ...
            initKey: function (token) {
                ...
                this.key = _.isUndefined(OC.getCurrentUser().uid) || _.isNull(OC.getCurrentUser().uid) ? "guest" : OC.getCurrentUser().uid;
                this.key = "chat-" + this.key + "-" + token;
            },

            save: function (msg) {

                var data = _.isUndefined(store.get(this.key)) ? {} : store.get(this.key);
                data.msg = msg;
            ...
            get: function () {
                return _.isUndefined(store.get(this.key)) ? "" : store.get(this.key).msg;
            },
            ...
            empty: function () {
                ...
                if (_.isUndefined(data) || _.isUndefined(data.msg)) {
                    return true;
                }
        ...
        onRender: function () {
            ...
            // Init msg draft key.
            self.msgDraft.initKey(this.collection.token);

            // Restore msg draft, if any.
            $message.text(self.msgDraft.get());

            $message.on('keydown input change', function (event) {
                self._onTypeComment(event, self);
            });
        ...
        _onTypeComment: function (ev, self) {
            var $field = $(ev.target);
...
})(OCA, OC, OCP, Marionette, autosize, moment, store, _);

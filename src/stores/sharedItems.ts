...
export const useSharedItemsStore = defineStore('sharedItems', () => {
		...
		for (const type of Object.keys(data)) {
			if (Object.keys(data[type]).length) {
				...
				for (const message of data[type]) {
					if (!sharedItemsPool[token][type][message.id]) {
	 ...
	 */
	function addSharedItemsFromMessages(token: string, type: string, messages: SharedItems[keyof SharedItems][]) {
		checkForExistence(token, type)
	 ...
	 * @param type type of shared item
	 */
	async function fetchSharedItems(token: string, type: string): Promise<{ hasMoreItems: boolean, messages: SharedItems[keyof SharedItems][] }> {

...
describe('gridLayout', () => {
	...
	describe('computeGridDimensions', () => {
		...
		test('returns no columns or rows when the grid has not been measured yet', () => {
			expect(computeGridDimensions({ ...fullGrid, gridWidth: 0, videoCount: 4 }))
				.toEqual({ columns: 0, rows: 0 })
			expect(computeGridDimensions({ ...fullGrid, gridHeight: 0, videoCount: 4 }))
				.toEqual({ columns: 0, rows: 0 })
		})

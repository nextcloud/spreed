...
import { DEMOTE_DELAY, PROMOTE_DELAY, useActiveSpeakers } from './useActiveSpeakers.ts'
...
describe('useActiveSpeakers', () => {
	...
	describe('slots', () => {
		test('promotes no more than four speakers at once', async () => {
			const models = Array.from({ length: 5 }, () => makeModel())
			const { promotedSessionIds } = createSpeakers(models)
			...
			expect(promotedSessionIds.value).toEqual(['session-0', 'session-1', 'session-2', 'session-3'])
		})

		test('lets a queued speaker in once a slot frees up', async () => {
			const models = Array.from({ length: 5 }, () => makeModel())
			const { promotedSessionIds } = createSpeakers(models)
			...
			expect(promotedSessionIds.value).not.toContain('session-4')

			// The first speaker goes silent and eventually makes room
			models[0].attributes.speaking = false
			await advance(DEMOTE_DELAY)

			expect(promotedSessionIds.value).toEqual(['session-1', 'session-2', 'session-3', 'session-4'])
		})

		test('does not evict a promoted speaker for a new one', async () => {
			const models = Array.from({ length: 5 }, () => makeModel())
			const { promotedSessionIds } = createSpeakers(models)
			...
			const promoted = [...promotedSessionIds.value]

			models[4].attributes.speaking = true
			await advance(PROMOTE_DELAY)

			expect(promotedSessionIds.value).toEqual(promoted)
		})
	})

		...
		<template v-else>
			...
			<ul
				...
				<template
					...
					<div
						v-if="isLastReadMessage(group.messages.at(-1))"
...
export default {
	...
	methods: {
		...
		messagesShouldBeGrouped(message1, message2) {
			...
			if ((message1.id === this.visualLastReadMessageId && message2.timestamp > message1.timestamp)
				|| (message2.id === this.visualLastReadMessageId && message1.timestamp > message2.timestamp)) {
				return false
			}
		...
		async generateSummary() {
			...
			await this.chatExtrasStore.requestChatSummary(this.message.token, this.visualLastReadMessageId)

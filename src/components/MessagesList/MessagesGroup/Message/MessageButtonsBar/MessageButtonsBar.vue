...
export default {
	...
	computed: {
		...
		messageDateTime() {
			return formatDateTime(this.message.timestamp * 1000, 'dateTimeShort')
		},

		editedDateTime() {
			return formatDateTime(this.message.lastEditTimestamp * 1000, 'dateTimeShort')
		},
			...
			return [
				{
					...
					label: t('spreed', 'Later today – {timeLocale}', { timeLocale: formatDateTime(laterTodayTime, 'timeShort') }),
					ariaLabel: t('spreed', 'Set reminder for later today'),
				},
				{
					key: 'tomorrow',
					timestamp: tomorrowTime,
					label: t('spreed', 'Tomorrow – {timeLocale}', { timeLocale: formatDateTime(tomorrowTime, 'weekdayShort timeShort') }),
					ariaLabel: t('spreed', 'Set reminder for tomorrow'),
				},
				{
					key: 'thisWeekend',
					timestamp: thisWeekendTime,
					label: t('spreed', 'This weekend – {timeLocale}', { timeLocale: formatDateTime(thisWeekendTime, 'weekdayShort timeShort') }),
					ariaLabel: t('spreed', 'Set reminder for this weekend'),
				},
				{
					key: 'nextWeek',
					timestamp: nextWeekTime,
					label: t('spreed', 'Next week – {timeLocale}', { timeLocale: formatDateTime(nextWeekTime, 'weekdayShort timeShort') }),
		...
		clearReminderLabel() {
			...
			return t('spreed', 'Clear reminder – {timeLocale}', { timeLocale: formatDateTime(this.currentReminder.timestamp * 1000, 'weekdayShort timeShort') })
		},
		...
		async setReminder(timestamp) {
			try {
				...
				showSuccess(t('spreed', 'A reminder was successfully set at {datetime}', {
					datetime: formatDateTime(timestamp, 'dateTimeLong'),
				}))
			} catch (error) {
				console.error(error)

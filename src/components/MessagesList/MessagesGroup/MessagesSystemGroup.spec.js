...
describe('MessagesSystemGroup.vue', () => {
	...
	test('renders grouped system messages', () => {
		...
		const { actorDisplayName, ...messageProps } = MESSAGES[0]
		...
		const { actorDisplayName: actorDisplayName1, ...messageProps1 } = MESSAGES[1]
	...
	describe('renders grouped system message of user actions', () => {
		...
		function testGroupedSystemMessages(wrapper, expectedMessage) {
			...
			expect(messagesEl.at(0).props().message).toBe(expectedMessage)
			expect(messagesEl.at(0).props().messageParameters).toStrictEqual({
				actor: {
					id: 'actor-1',
					displayName: 'actor one',
					type: ATTENDEE.ACTOR_TYPE.USERS,
			...
			expect(messagesEl.at(3).props().message).toBe(MESSAGES[2].message)
			expect(messagesEl.at(3).props().messageParameters).toStrictEqual({
				actor: {
					id: 'actor-3',
					displayName: 'actor three',
					type: ATTENDEE.ACTOR_TYPE.USERS,

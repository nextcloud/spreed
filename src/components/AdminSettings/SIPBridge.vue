		...
		<template v-else>
			...
			<template v-if="dialOutEnabled && !dialOutAnonymous">
				...
				<NcTextField
					...
					:disabled="loading"
					:placeholder="t('spreed', '+49123456789')"
					label-outside />
				...
				<NcTextField
					...
					:disabled="loading"
					:placeholder="t('spreed', 'Dial-out prefix')"
					label-outside />
				<p class="settings-hint additional-top-margin">
					{{ t('spreed', 'Prefix to configured user number for outgoing calls') }}
				</p>
			</template>
	...
	methods: {
		...
		async saveSIPSettings() {
			...
			if (this.currentSetup.dialOutAnonymous !== this.dialOutAnonymous) {
				await OCP.AppConfig.setValue('spreed', 'sip_bridge_dialout_anonymous', this.dialOutEnabled)
			}

...
const listItems = computed<VirtualListItem[]>(() => {
	if (!props.showTags || tagsStore.sortedTags.length === 0) {
		// tags are hidden or not known yet, fall back to a flat list
	...
	if (!groupedConversations.hasTagged && groupedConversations.favorites.length === 0) {
		// no custom tags and favorites, fall back to a flat list
		return props.conversations
	}

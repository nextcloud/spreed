...
onMounted(() => {
	timer.value = setInterval(computeElapsedTime, 1000)
})
...
const eventDateLabel = computed(() => {
	...
	const startDateString = startDate.toLocaleString(undefined, { hour: '2-digit', minute: '2-digit' })
	const endDateString = endDate.toLocaleString(undefined, { hour: '2-digit', minute: '2-digit' })
...
/**
 * Computes the time elapsed since the start of the call
 *
 */
function computeElapsedTime() {
	if (props.eventRoom.roomActiveSince === null) {
		elapsedTime.value = '--:--'
		return
	}
	elapsedTime.value = formattedTime(Number(new Date()) - props.eventRoom.roomActiveSince * 1000)
}

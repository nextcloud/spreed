...
const eventDateLabel = computed(() => {
	...
	if (startDate.toDateString() === endDate.toDateString()) {
		if (isToday.value || isTomorrow) {
			...
			const relativeFormatter = new Intl.RelativeTimeFormat(getCanonicalLocale(), { numeric: 'auto' })
...
const attachmentInfo = computed(() => {
	if (totalAttachments.value.length > 0) {
...
const roomLabel = computed(() => {
	return props.eventRoom.roomType === CONVERSATION.TYPE.ONE_TO_ONE
		? t('spreed', 'With {displayName}', { displayName: props.eventRoom.roomDisplayName }, { escape: false })
		: t('spreed', 'In {conversation}', { conversation: props.eventRoom.roomDisplayName }, { escape: false })
})
...
<template>
	<div class="event-card"
		...
		<template v-if="totalAttachments.length > 0 && attachmentInfo">
...
.event-card {
	...
	// If the card is highlighted, use the highlighted background
	&.event-card--highlighted:not(.event-card--in-call):hover > .event-card__invitation-info.hovered {
		background-color: var(--color-primary-light);
	}

...
const canCreatePollDrafts = computed(() => {
	...
	return hasTalkFeature(token.value, 'talk-polls-drafts')
		&& (participantType === PARTICIPANT.TYPE.OWNER || participantType === PARTICIPANT.TYPE.MODERATOR)
		&& (type === CONVERSATION.TYPE.GROUP || type === CONVERSATION.TYPE.PUBLIC)
})
...
const openPollDraftHandler = ({ selector }: Events['poll-drafts-open']) => {
	container.value = selector
	showPollDraftHandler.value = true
}

const openPollEditor = ({ id, fromDrafts, selector }: Events['poll-editor-open']) => {
	container.value = selector

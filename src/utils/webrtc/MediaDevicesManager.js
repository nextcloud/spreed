...
export default function MediaDevicesManager() {
	...
	this.isNavigatorSelectAudioOutputSupported = !!navigator?.mediaDevices?.selectAudioOutput
	this.isAudioContextSetSinkIdSupported = !!(new AudioContext().setSinkId)
	this.isAudioElementSetSinkIdSupported = !!(new Audio().setSinkId)
	/**
	 ...
	 * - supports navigator.mediaDevices.selectAudioOutput()
	 * Safari:
	 * - does not support audio output selection: https://bugs.webkit.org/show_bug.cgi?id=216641
	 *
	 * @return {boolean} true if supported, false otherwise.
	 */
	this.isAudioOutputSelectSupported = this.isNavigatorSelectAudioOutputSupported || this.isAudioContextSetSinkIdSupported || this.isAudioElementSetSinkIdSupported

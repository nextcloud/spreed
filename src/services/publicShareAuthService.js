 ...
 */
const getPublicShareAuthConversation = async function(shareToken) {
	const response = await axios.post(generateOcsUrl('apps/spreed/api/v1', 2) + `publicshareauth?shareToken=${shareToken}`)
	return response.data.ocs.data.token
}

export {
	getPublicShareAuthConversation,
}

...
class ConversationCategoryMapper extends QBMapper {
	...
	public function getMaxSortOrder(string $userId): int {
		...
		$qb->select($qb->func()->max('sort_order'))
			->from($this->getTableName())
			->where($qb->expr()->eq('user_id', $qb->createNamedParameter($userId)))
			->andWhere($qb->expr()->eq('type', $qb->createNamedParameter(ConversationCategory::TYPE_CUSTOM)));
	 ...
	 */
	public function findBuiltInByType(string $userId, string $type): ConversationCategory {
		$qb = $this->db->getQueryBuilder();
		$qb->select('*')
			->from($this->getTableName())
			->where($qb->expr()->eq('user_id', $qb->createNamedParameter($userId)))

...
class ChatController extends AEnvironmentAwareController
{
    ...
    public function sendMessage(
        ...
        if ($this->userId === null) {
            $actorType = 'guests';
            $sessionId
                       = $this->session->getSessionForRoom($this->room->getToken());
        ...
        return new DataResponse([
            ...
            'timestamp'         => $comment->getCreationDateTime()
                ->getTimestamp(),
            'message'           => $chatMessage->getMessage(),
            'messageParameters' => $chatMessage->getMessageParameters(),
            'systemMessage'     => $chatMessage->getMessageType() === 'system'
                ? $comment->getMessage() : '',
        ], Http::STATUS_CREATED);
    }
    ...
    public function receiveMessages(
        ...
        foreach ($comments as $comment) {
            ...
            $messages[] = [
                ...
                'systemMessage'     => $message->getMessageType() === 'system'
                    ? $comment->getMessage() : '',
            ];
        }
     ...
     * @PublicPage
     *
     * Get rooms info and lastest comment per room.
     *
     * @param $roomsInfo  - {token: "xyz", last_message: 1}
     *
     * @return DataResponse
     */

...
class GuestManager {
	...
	public function sendEmailInvitation(Room $room, Participant $participant): void {
		...
		if ($user instanceof IUser) {
			$subject = $this->l->t('%s invited you to conversation "%s".', [$user->getDisplayName(), $room->getDisplayName('')]);
		...
		$template->addHeader();
		$template->addHeading(htmlspecialchars($room->getDisplayName('')), $room->getDisplayName(''));
		...
		if ($room->getLobbyState() !== Webinary::LOBBY_NONE && $room->getLobbyTimer() !== null) {
			$template->addBodyText($this->l->t('Click the button below to join the lobby now.'));
			$template->addBodyButton(
				$this->l->t('Join lobby for "%s"', [$room->getDisplayName('')]),
				$link
			);
		} else {
			$template->addBodyText($this->l->t('Click the button below to join the conversation now.'));
			$template->addBodyButton(

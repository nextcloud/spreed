OC.L10N.register(
    ...
    "- Moderators can now create breakout rooms (requires the High-performance backend)" : "- 主催者がブレイクアウトルームを作成できるようになりました(要高性能バックエンド)",
    "- Calls can now be recorded (requires the High-performance backend)" : "- 通話を録音できるようになりました(要高性能バックエンド)",
    ...
    "- Messages can now be edited by logged-in authors and moderators for 6 hours" : "- ログインした作成者とモデレーターが6時間以内はメッセージを編集できるようになりました",
    "- Unsent message drafts are now saved in your browser" : "- 未送信のメッセージの下書きはブラウザに保存されるようになりました",
    "- Text chatting can now be done in a federated way with other Talk servers" : "- 他のNextcloudのTalkサーバーと連携してテキストチャットできるようになりました",
    "- Moderators can now ban accounts and guests to prevent them from rejoining a conversation" : "- モデレータはアカウントとゲストを退室させ、会話に再度参加するのを禁止できるようになりました",
    "- Upcoming calls from linked calendar events and out-of-office replacements are now shown in conversations" : "- リンクされたカレンダーイベントや不在時の代理者からの通話が、会話画面に表示されるようになりました",
    "- Calls can now be done in a federated way with other Talk servers (requires the High-performance backend)" : "- 他のNextcloud Talk サーバーと連携して通話できるようになりました(要高性能バックエンド)",
    "- Introducing the Nextcloud Talk desktop client for Windows, macOS and Linux: %s" : "- Windows、macOS、Linux用Nextcloud Talkデスクトップ クライアントをリリース: %s",
    "- Summarize call recordings and unread messages in chats with the Nextcloud Assistant" : "- Nextcloudアシスタントを使用して、チャットの通話録音と未読メッセージを要約",
    "- Improved meetings with recognizing guests invited via their email address, import of participant lists, drafts for polls and downloading call participant lists" : "- 電子メールアドレス経由で招待されたゲストを識別、参加者リストのインポート、投票の下書き、および通話参加者リストのダウンロードによる会議の改善",
    "- Archive conversations to stay focused" : "- 会話をアーカイブして、必要なものに集中できるようになりました",
    "- Schedule a meeting into your calendar from within a conversation" : "- 会話内からカレンダーへ会議を予約する",
    "- Search for messages of the current conversation directly in the right sidebar" : "- 現在の会話のメッセージを右側のサイドバーで直接検索",
    "- See more conversations on a first glance with the new compact list (enable in the Talk settings)" : "- 新しいコンパクトなリストを使用して、より多くの会話を一目で確認できます (トーク設定で有効化)",
    "- Meeting conversations now sync the title and description from the calendar and are hidden with a search filter until they are close to the start" : "- 会議についての会話は、カレンダーでタイトルと説明が同期されるようになりました。開始時間が近づくまで、検索フィルターでは非表示になります",
    "- Mark conversations as sensitive in the notification settings, to hide the message content from the conversation list and notifications" : "- 通知設定で会話を機密としてマークすると、会話リストと通知からメッセージの内容が非表示になります",
    "- To receive push notifications during \"Do not disturb\", mark conversations as important" : "- 会話に重要マークを付けると「応答不可」中にもプッシュ通知を受信できます",
    "- Add other participants to a one-to-one call to create a new group call on the fly" : "- この1対1通話に他の参加者を追加して、その場で新しいグループ通話を作成する",
    "- Use threads to keep your chat and discussions organized" : "- スレッドを使ってチャットや議論を整理",
    "- Live transcriptions now available during the call (requires the live-transcription ExApp and the High-performance backend)" : "- 通話中にライブ文字起こしが利用できるようになりました (ライブ文字起こしのExAppと高性能バックエンドが必要です)",
    ...
    "You accepted the invitation" : "あなたはこの招待を受け入れました",
    "{actor} invited you" : "{actor} さんがあなたを招待しました",
    "An administrator invited you" : "管理者があなたを招待しました",
    ...
    "You removed team {circle}" : "{circle}を削除しました",
    "An administrator removed team {circle}" : "管理者がチーム{circle}を削除しました",
    ...
    "Call summary now available" : "通話要約が利用可能になりました",
    "The summary for the call in {call} was uploaded to {file}." : "{call}通話の要約が {file}にアップロードされました。",
    ...
    "You missed a group call in {call}" : "{call} からのグループ着信に応答できませんでした。",
    ...
    "Federation" : "連携",
    "It is highly recommended to configure \"memcache.locking\" when Talk Federation is enabled." : "Talk連携を有効にする場合は、\"memcache.locking\"の構成が必須です。",
    ...
    "Error: System times of Nextcloud server and High-performance backend server are out of sync. Please make sure that both servers are connected to a time-server or manually synchronize their time." : "エラー: Nextcloudサーバーと高性能バックエンドサーバーのシステム時間が同期していません。両方のサーバーがタイムサーバーに接続されていることを確認するか、手動で時刻を同期してください。",
    ...
    "It is highly recommended to configure a memory cache when running Nextcloud Talk with a High-performance backend." : "高性能バックエンドでNextcloud Talkを実行する場合は、メモリキャッシュを構成することを強くお勧めします。",
    ...
    "End-to-end encrypted calls with a configured SIP bridge require a newer version of the High-performance backend and SIP bridge." : "構成されたSIPブリッジでのエンドツーエンドの暗号化通話には、新しいバージョンの高性能バックエンドとSIPブリッジが必要です。",
    ...
    "If your high-performance backend account includes STUN and/or TURN functionality, the settings will be updated accordingly." : "高性能バックエンド アカウントにSTUNおよび/またはTURN機能が含まれている場合、設定はそれに応じて更新されます。",
    ...
    "Error: System times of Nextcloud server and Recording backend server are out of sync. Please make sure that both servers are connected to a time-server or manually synchronize their time." : "エラー: Nextcloud サーバーと録画バックエンドサーバーのシステム時刻が同期していません。両方のサーバーがタイムサーバーに接続されていることを確認するか、手動で時刻を同期してください。",
    ...
    "Recording backend configuration is only possible with a High-performance backend." : "録画バックエンドは、高パフォーマンスのバックエンドでのみ可能です。",
    ...
    "Please note that in calls with more than 2 participants without the High-performance backend, participants will most likely experience connectivity issues and cause high load on participating devices." : "高性能バックエンドを使用しない3人以上の参加者との通話では、参加者に接続の問題が発生し、参加デバイスに高負荷がかかる可能性が高いことに注意してください。",
    "Install the High-performance backend to ensure calls with multiple participants work seamlessly." : "複数の参加者との通話がシームレスに機能するようにするには高性能バックエンドをインストールしてください。",
    "The High-performance backend is required for calls and conversations with multiple participants. Without the backend, all participants have to upload their own video individually for each other participant, which will most likely cause connectivity issues and a high load on participating devices." : "複数参加者の通話や会話には高性能バックエンドが必要です。バックエンドがない場合、参加者全員が他の参加者全員に対して一対一で動画データを送信する必要があり、接続問題や参加デバイスの高負荷を引き起こす可能性が非常に高くなります。",
    "It is highly recommended to set up a distributed cache when using Nextcloud Talk with a High-performance backend." : "Nextcloud Talkを高性能バックエンドで使用する場合は、分散キャッシュをセットアップすることを強くお勧めします。",
    "Add High-performance backend server" : "高性能バックエンドを追加",
    ...
    "Now" : "現在",
    "Invite {user}" : "{user} を招待",
    "Meeting created" : "会議を作成しました",
    "Upcoming meetings" : "今後の会議",
    "No upcoming meetings" : "今後の会議はありません",
    ...
    "Calendar" : "カレンダー",
    "Attendees" : "参加者",
    "No other participants to send invitations to." : "他に招待状を送信できる参加者はいません。",
    "Save" : "保存する",
    ...
    "Password could not be copied" : "パスワードをコピーできませんでした",
    "Guest access" : "ゲスト参加",
    ...
    "Set language spoken in calls" : "通話時の言語を設定する",
    "Languages could not be loaded" : "言語を読み込めませんでした",
    "Loading languages …" : "言語を読み込んでいます ...",
    "Invalid language" : "無効な言語",
    "Default language (English)" : "デフォルト言語（英語）",
    "Live transcription language set: {languageName}" : "ライブトランスクリプション言語セット:{languageName}",
    "Start time: {date}" : "開始時刻: {date}",
    ...
    "Import email participants" : "メール参加者をインポート",
    "You can import a list of email participants from a CSV file." : "CSVファイルで参加者のメールアドレスリストをインポートできます。",
    "Poll drafts" : "アンケートの草稿",
    "Browse poll drafts" : "アンケート草稿を閲覧",
    ...
    "Log content" : "ログ情報",
    "Only moderators are allowed to mention @all" : "@allにメンションできるのはモデレーターのみです",
    "All participants are allowed to mention @all" : "すべての参加者が@allにメンションできます。",
    "Participants are now allowed to mention @all." : "参加者は@allにメンションできるようになりました。",
    "Mentioning @all has been limited to moderators." : "@allのメンションはモデレーターのみに限定されています。",
    "Allow participants to mention @all" : "参加者が@allにメンションできるようにします",
    "Mention permissions" : "メンション権限",
    "Notifications" : "通知",
    "Notify about calls in this conversation" : "この会話で呼び出し時に通知する",
    "Important conversation" : "重要な会議",
    "Sensitive conversation" : "プライベートな会話",
    "Message preview will be disabled in conversation list and notifications" : "会話リストと通知ではメッセージのプレビューが無効になります",
    ...
    "View event on Calendar" : "カレンダーでイベントを表示",
    "Error while creating the conversation" : "会話の作成中にエラーが発生しました",
    "Hello, {displayName}" : "こんにちは、{displayName} さん",
    "Start meeting now" : "今すぐ会議を始める",
    "Give your meeting a title" : "会議にタイトルをつける",
    "Create and copy link" : "リンクを作成してコピー",
    "New conversation" : "新しい会話",
    "Call a phone number" : "電話番号にかける",
    "Scroll backward" : "後方にスクロール",
    "Scroll forward" : "前方にスクロール",
    "Schedule meetings" : "会議を予約",
    "You don't have any upcoming meetings" : "今後の会議はありません",
    "Open calendar" : "カレンダーを開く",
    "Unread mentions" : "未読の返信",
    "Messages where you were mentioned will show up here. You can mention people by typing @ followed by their name" : "あなたがメンションされたメッセージはここに表示されます。@に続けて相手の名前を入力すると、メンションできます。",
    "Upcoming reminders" : "今後のリマインダー",
    "Message reminders" : "メッセージのリマインダー",
    "Set a reminder on a message to be notified" : "チャットメッセージにリマインダーを設定すると後で再度通知されるようにできるようになりました",
    "Start a group conversation" : "グループ会話を始める",
    "Create conversation" : "会話を作成",
    "Enter your name" : "あなたの名前を入力",
    "Submit name and join" : "名前を送信して参加する",
    "Do you already have an account?" : "すでにアカウントをお持ちですか?",
    "Log in" : "ログイン",
    "Uploaded file is verified" : "アップロードされたファイルは検証済みです",
    ...
    "Ignore \"Do not disturb\"" : "「邪魔しないでください」を無視する",
    ...
    "Tags" : "タグ",
    "Notify about calls" : "着信について通知",
    "Hide message text" : "メッセージテキストを非表示",
    "Rename tag" : "タグ名を変更",
    "Delete tag" : "タグを削除",
    "Move up" : "上に移動",
    "Move down" : "下に移動",
    ...
    "Select speaker" : "発言者を指定",
    "No speaker available" : "指定できる発言者がいません",
    "Select camera" : "カメラを選択",
    "No camera available" : "利用可能なカメラがありません",
    "Select a device" : "デバイスを選択する",
    "Test speakers" : "発言者をテスト",
    "Test" : "テスト",
    "Devices" : "端末",
    "Backgrounds" : "背景",
    "No audio" : "音声なし",
    "No camera" : "カメラがありません",
    "Display video as you will see it (mirrored)" : "鏡像表示でプレビューを表示",
    ...
    "Create a thread silently" : "非通知でスレッドを作成する",
    ...
    "Calls without High-performance backend can cause connectivity issues and high load on devices. {linkstart}Learn more{linkend}" : "高性能バックエンドを使用しない通話では、接続の問題が発生し、デバイスに高負荷がかかる可能性があります。{linkstart}詳細{linkend}",
    ...
    "Search in {name}" : "{name}から検索",
    "{actor} in {conversation}" : "{conversation} の {actor}",
    "No results found" : "結果が見つかりません",
    "Load more results" : "結果をさらに読み込む",
    "Projects" : "プロジェクト",
    "No shared items" : "共有アイテムがありません",
    "Thread notifications" : "スレッド通知",
    ...
    "Compact conversations list" : "会話リストをシンプルに表示",
    "Play sounds when participants join or leave a call" : "参加者が通話に参加したとき、または通話を離れたときにサウンドを再生する",
    "Currently not available on iPhone and iPad due to technical restrictions by the manufacturer" : "現在、メーカーによる技術的制限によりiPhoneおよびiPadではご利用いただけません。",
    "Notification settings" : "通知設定",
    "Sounds for chat and call notifications" : "チャットや着信時の通知",
    ...
    "Turn camera and microphone off by default" : "自動的にカメラとマイクをオフにする",
    "Blur camera background by default" : "自動的に背景をぼかす",
    "Skip device preview before joining a call" : "通話に参加する前のデバイスのプレビューをスキップする",
    "Appearance & Sounds" : "外観と通知",
    "Privacy" : "プライバシー",
    "Send read receipts" : "既読ステータス",
    "When off, all read statuses will be hidden" : "この機能をオフにすると、すべての既読ステータスが非表示になります。",
    "Share typing status" : "入力状態を共有",
    "When off, all typing indicators will be hidden" : "この機能をオフにすると、すべての入力インジケーターが非表示になります。",
    ...
    "Zoom-in / zoom-out a screen share" : "画面共有のズームイン/ズームアウト",
    ...
    "Join a conversation or start a new one" : "会話に参加するか、新しいのを開始",
    ...
    "Error fetching upcoming reminders" : "今後のリマインダー取得エラー",
    ...
    "Running the High-performance backend \"conversation_cluster\" mode is deprecated and will no longer be supported in the upcoming version. The High-performance backend supports real clustering nowadays which should be used instead." : "高パフォーマンス バックエンドの「conversation_cluster」モードの実行は非推奨となり、次のバージョンではサポートされなくなります。現在、高性能バックエンドはクラスタリングをサポートしているため、それを代わりに使用してください。",
    "Next meeting" : "次の会議",
    "Collapse stripe" : "スレッドを折りたたむ",
    "Expand stripe" : "スレッドを展開する",
    "Schedule a meeting from your calendar. A Talk conversation needs to be set as location to show up here" : "カレンダーに会議を予約します。ここに表示するためには、会議の場所にTalkのチャットルームを指定してください。",
